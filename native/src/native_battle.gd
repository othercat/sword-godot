...
static func command(package, state: Dictionary, action: String, target: String = "", skill_id: String = "") -> Dictionary:
	...
	elif action != "guard": return {"error": "unsupported battle action"}
	...
	for row in battle.enemies:
		...
		_hit(package, row, defender, defender.instance_id in battle.guarding, battle.events)
		if _living_turn(state, battle.party, 0) < 0: return {"outcome": "loss"}
...
static func _hit(package, source: Dictionary, target: Dictionary, guarded: bool, events: Array) -> void:
	var attack: int = package.index.actor_definitions[source.definition_id].combat.attack
	var defense: int = package.index.actor_definitions[target.definition_id].combat.defense
