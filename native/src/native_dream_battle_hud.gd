...
func _number(value: int, at: Vector2, right: bool, tint: Color) -> void:
	var text: String = str(value)
	if right: at.x += 30-text.length()*6
	var font: Font = get_theme_font("font")
	for digit in text:
		draw_string(font,at+Vector2(0,7),digit,HORIZONTAL_ALIGNMENT_LEFT,6,8,tint); at.x += 6
...
func _draw() -> void:
	...
	for id in cards:
		...
		draw_style_box(_panel(id == active_id),card.panel_rect)
		...
		_number(card.hp,p+Vector2(19,5),true,Color("ead28e")); _number(card.stats.max_hp,p+Vector2(49,10),false,Color("ead28e"))
		_number(card.mp,p+Vector2(19,21),true,Color("7dd3d2")); _number(card.stats.max_mp,p+Vector2(49,26),false,Color("7dd3d2"))
		for at in [Vector2(47,6),Vector2(47,22)]: draw_line(p+at+Vector2(4,0),p+at+Vector2(0,7),Color("8cb8b0"),1)
...
func clear() -> void:
	clear_commands(); cards.clear(); _view = null; active_id = ""; queue_redraw()
