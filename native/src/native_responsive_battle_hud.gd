...
func _draw() -> void:
	...
	for id in cards:
		...
		if not card.portrait_asset.is_empty():
			...
			draw_texture_rect(texture,Rect2(face.get_center()-extent/2,extent),false,Color(.45,.45,.45) if card.hp == 0 else Color.WHITE)
		...
		draw_string(font,Vector2(left,panel.position.y+(profile.style.font_size+3)*unit),card.name,HORIZONTAL_ALIGNMENT_LEFT,width,font_size,Color(colors.text))
		var row: int = 0
		for entry in [["HP",card.hp,card.stats.max_hp,colors.hp],["MP",card.mp,card.stats.max_mp,colors.mp]]:
			...
			draw_rect(bar,Color(colors.track))
			var ratio: float = clampf(float(entry[1])/maxf(1,float(entry[2])),0,1)
			draw_rect(Rect2(bar.position,Vector2(width*ratio,height)),Color(entry[3]))
			...
			draw_string(font,at+Vector2(0,unit),text,HORIZONTAL_ALIGNMENT_LEFT,width-4*unit,value_size,Color(colors.track))
			draw_string(font,at,text,HORIZONTAL_ALIGNMENT_LEFT,width-4*unit,value_size,Color(colors.text))
			row += 1
