...
static func make_flat(map_data: Dictionary, layer: Dictionary, textures: Dictionary, atlas_cache: Dictionary) -> Node2D:
	...
	for placement in layer.placements:
		...
		result.set_cell(Vector2i(placement.position.x, placement.position.y) - origin, atlas_cache.source_ids[placement.tile_id], Vector2i.ZERO)
	return result
...
static func _make_atlas(map_data: Dictionary, definitions: Dictionary, textures: Dictionary, basis: Vector2) -> Dictionary:
	...
	for tile in definitions.values():
		...
		var size = Vector2i(2 * maxi(tile.anchor.x, tile.width - tile.anchor.x), 2 * maxi(tile.anchor.y, tile.height - tile.anchor.y))
