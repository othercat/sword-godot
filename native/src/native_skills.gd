...
static func plan(package, state: Dictionary, source: Dictionary, skill_id: String, target: String) -> Dictionary:
	...
	var result: Dictionary = Effects.plan(state, skill, target)
...
static func validate_events(package, state: Dictionary) -> String:
	...
	var command: Dictionary = battle.events[0]; var skill: Dictionary = definition(package, command.get("skill_id", ""))
	var sources: Array = state.entities.filter(func(a): return a.instance_id == command.source)
