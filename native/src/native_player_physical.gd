...
static func validate_state(package, state: Dictionary) -> String:
	...
	for key in ["last_battle","pending"]:
		...
		elif not ledger.outcome.is_empty() or ledger.rng_start != end: return "physical pending random continuation mismatch"
		...
		for action in ledger.actions:
			if action.step <= previous_step or action.source not in ledger.party or action.rng_before != rng_cursor: return "physical action source, step or random gap"
	...
	if cursor(state.rng) != end: return "physical random cursor differs from action ledger"
