...
func _initialize() -> void:
	...
	check(fade.get("completed") == false and fade.get("prepared") == true
		and fade.receipt.first == 1 and fade.receipt.second == 2,
		"cross-fade prepares separate pages without claiming lane execution")
	check(str(fade.receipt.boundary).contains("not implemented"),
		"lane execution and final presentation remain a named boundary")
	check(renderer.receipts().size() == count and renderer.current_rgba() == first.rgba,
		"preparation does not publish renders or overwrite the current page")
