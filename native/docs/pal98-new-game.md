...
- `begin()` drives the real reload/enter chain. The intro runs scene 1's own
  entry, which requests scene 2; the chain loads MAP20 and MAP12, renders
  both backgrounds through the real pass and rests on scene 2 awaiting a real
  player. A further scene entry stops the intro by name instead of inventing
  player input. Display routing is real: palette family through the display
  executor, capture/restore through the shared indexed page owner, map
  backgrounds through the renderer.
